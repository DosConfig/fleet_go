...
class DriverScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final activeTripAsync = ref.watch(watchActiveDriverTripProvider(user.uid));
      ...
      data: (activeTrip) {
        ...
        return Scaffold(
          ...
          body: activeTripId == null
              ? _ProposedList(onAccept: (tripId) => _acceptTrip(ref, context, tripId, user.uid))
              : _ActiveTripView(tripId: activeTripId),
        );
      },
  ...
  Future<void> _acceptTrip(WidgetRef ref, BuildContext context, String tripId, String driverId) async {
    try {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return tripAsync.when(
      data: (state) {
        ...
        return _ActiveTripContent(tripId: tripId, state: state);
      },
      loading: () => const Center(child: CircularProgressIndicator()),
      error: (e, _) => Center(child: Text('오류: $e')),
    );
...
class _ActiveTripContent extends ConsumerWidget {
  const _ActiveTripContent({required this.tripId, required this.state});
    ...
    return Column(
      children: [
        ...
        _TripControlPanel(
          ...
          onStopGps: () => ref.read(driverLocationSenderProvider.notifier).stop(),
        ),
      ],
    );
  }
    ...
    return Container(
      ...
      child: SafeArea(
        ...
        child: Column(
          ...
          children: [
            Text(_statusLabel(state), style: Theme.of(context).textTheme.titleMedium),
            ...
            if (nextEvent != null)
              SizedBox(
                ...
                child: FilledButton(
                  ...
                  child: Text(_eventLabel(nextEvent)),
                ),
              ),
            if (_isTerminal)
              SizedBox(
